...
async def stream_chat(
    ...
    logger.warning("!!!!!!!!!!!!!!! SMART ROUTER STREAM CHAT FUNCTION RUNNING WITH UPDATED CODE !!!!!!!!!!!!!!!")
    try:
        ...
        try:
            as_dict = request.dict() if hasattr(request, 'dict') else {"unable_to_get_dict": True}
        ...
        try:
            ...
            logger.info(f"Direct attribute access - perform_web_search: {web_search_flag}")
        except Exception as e:
        ...
        try:
            if hasattr(request, '__dict__') and 'perform_web_search' in request.__dict__:
                ...
                logger.info(f"Web search flag from __dict__: {perform_web_search}")
            elif hasattr(request, 'perform_web_search'):
                perform_web_search = request.perform_web_search
                logger.info(f"Web search flag from attribute: {perform_web_search}")
            else:
        ...
        if not perform_web_search and ('search' in user_message.lower() or 'find' in user_message.lower()):
            logger.info(f"Forcing web search based on message content: '{user_message}'")
            perform_web_search = True
        
        logger.info(f"FINAL web search flag: {perform_web_search}")
        
        # Web search specific logging
        logger.info("================== WEB SEARCH DECISION LOGIC ==================")
        logger.info(f"FORCE_WEB_SEARCH global flag: {FORCE_WEB_SEARCH}")
        logger.info(f"getattr request.perform_web_search: {getattr(request, 'perform_web_search', False)}")
...
"""
                    ...
                    system_prompt = getattr(request, 'system_prompt', DEFAULT_SYSTEM_PROMPT)
                    messages = [
                    ...
                    logger.info("ROUTING TO GENERAL KNOWLEDGE WITH SEARCH RESULTS")
                    ...
                    modified_request_data = {
                        ...
                        "stream": True,
                        "system_prompt": search_instruction  # Pass the search instruction as system prompt
                    }
                    ...
                    modified_request = GeneralChatRequest(**modified_request_data)
